...
def ElectricalConnector(port1: DevicePort, port2: DevicePort):
    xpts, ypts = ElbowRouter(
        port1=port1, port2=port2, offset=ElectricalConnectorOptions["elbow_offset"]
    )
    ...
    return sm.make_tapered_path(
        xpts, ypts, widths, layer=ElectricalConnectorOptions["metal_layer"]
    )
...
class FreeFreeMembraneELE(FreeFreeMembrane):
    ...
    def geom(self):
        ...
        conn_w = 0.8
        port1 = ElectricalPort(
            x0=0, y0=S / 2, orient="north", width=conn_w, name="emesa"
        )
        ...
        pad = sm.make_rounded_rect(
            x0=0, y0=S / 2 - 5, width=S - 10, height=4, corner_radius=1
        )
        pad += sm.make_path(
            xpts=[0, 0], ypts=[S / 2 - 5, S / 2], width=conn_w, to_poly=True
        )
