...
geomE = sm.make_rect(x0=0, y0=2, width=6, height=3, numkey=3, layer=2)

# Circles:
# A circle in (10,0) with radius 3 um
geomE += sm.make_circle(x0=10, y0=0, r=3)
...
geomE += sm.make_circle(x0=20, y0=0, r=3, layer=1, to_poly=True, vertices=50)


# Polygons:
# Arbitrary shaped polygons (but avoid self-intersections)
geomE += sm.make_poly(xpts=[30, 40, 35, 31], ypts=[0, -2, 8, 7])

# Path:
# Open piece-wise paths with a width
geomE += sm.make_path(xpts=[50, 50, 60], ypts=[0, 10, 10], width=1, to_poly=True)
...
# Text:
geomE += sm.make_text(
    x0=70, y0=0, text="Hello", height=10, width=1, numkey=1, to_poly=True
)
...
geomE += sm.make_rounded_rect(
    x0=0, y0=20, width=10, height=5, corner_radius=1, resolution=8
)
# Creates a rectangle with 1 um radius rounded edges.

# Ellipse
geomE += sm.make_ellipse(x0=15, y0=20, rX=3, rY=2, rot=45, to_poly=True)
...
# Rings
geomE += sm.make_ring(x0=30, y0=20, rX=5, rY=5, rot=0, w=0.5, to_poly=True)
...
# Arcs
geomE += sm.make_arc(x0=40, y0=20, rX=5, rY=5, rot=0, w=0.5, a1=0, a2=90, to_poly=True)
...
geomE += sm.make_tapered_path(xpts=[50, 55, 60], ypts=[15, 20, 23], widths=[1, 0.5, 2])
