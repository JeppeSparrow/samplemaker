...
elist = [
    smdev.NetListEntry(
        "X", 0, 0, "E", {"AAA": "input", "BBB": "link"}, {"NETLIST": netlist}
    ),
    smdev.NetListEntry(
        "X", 40, 12, "E", {"AAA": "link", "BBB": "output"}, {"NETLIST": netlist}
    ),
]
...
tab.set_table_positions(
    tab.Regular(rows=1, cols=10, ax=40, ay=15, bx=0, by=0)
)  # ax is 40 here, not 30 as in the comment
